...
import { FeishuNotifier, WebhookPayload } from './feishuNotifier';
...
function getNotifier(): FeishuNotifier | null {
  const cfg = vscode.workspace.getConfiguration('feishuNotifier');
  const webhookEndpoint = cfg.get<string>('webhookEndpoint', '').trim();
  const triggerToken = cfg.get<string>('triggerToken', '').trim();
  if (!webhookEndpoint || !triggerToken) return null;
  return new FeishuNotifier({ webhookEndpoint, triggerToken });
}

function showConfigError() {
  vscode.window
    .showErrorMessage(
      '飞书汇报器：请先配置 webhookEndpoint 和 triggerToken。',
      '打开设置'
    )
    .then((v) => {
      if (v === '打开设置') {
...
async function generateSummary(
  ...
  try {
    ...
    if (models.length === 0) return userContent;
...
async function reportNow() {
  const notifier = getNotifier();
  if (!notifier) { showConfigError(); return; }
  ...
  const summary = await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: '飞书汇报器', cancellable: false },
    async (progress) => {
...
async function testConnection() {
  ...
  try {
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: '飞书汇报器：正在测试连接...' },
      () => notifier.test()
    );
...
async function endSession() {
  ...
  try {
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: '飞书汇报器：正在生成会话总结...' },
      () => sendSessionEndSummary()
    );
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('feishu-notifier.endSession', endSession),
    vscode.commands.registerCommand('feishu-notifier.testConnection', testConnection),
    vscode.commands.registerCommand('feishu-notifier.settings', () =>
      vscode.commands.executeCommand('workbench.action.openSettings', 'feishuNotifier')
    ),
  );
...
async function readLatestChatSession(): Promise<string | null> {
  try {
    ...
    for (const chatSessionDir of possiblePaths) {
      if (!fs.existsSync(chatSessionDir)) continue;
...
function extractChatHistory(sessionContent: string): string {
  try {
      ...
      .map((msg: any, idx: number) => {
        ...
        if (msg.message) {
          if (typeof msg.message === 'string') {
            ...
            text = msg.message.map((m: any) => m.text || m.content || m.value || '').join('\n');
          }
        } else if (msg.prompt) {
          text = msg.prompt;
...
async function sendSessionEndSummary(): Promise<void> {
  const notifier = getNotifier();
  if (!notifier) throw new Error('飞书集成未配置');
      ...
      summary = await vscode.window.withProgress(
        ...
        (progress) => {
          ...
          return generateSummary(
            `请根据以下本次 VS Code Chat 工作会话的完整对话，总结我的工作成果和进展。` +
