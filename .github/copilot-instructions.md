...
**每次完成用户的任务请求后，必须调用 `feishu_notify` MCP 工具，将本次工作总结发送到飞书群组。**

- **触发时机**：完成一个完整任务时（代码修改完毕、问题解决后、功能实现后）
- **调用方式**：直接调用 `feishu_notify` 工具，无需用户提示
- **总结内容**：用中文简洁描述本次完成的主要工作、关键改动和结果
- **标题**：简短概括本次任务，例如 `"📝 完成集成管理功能"` 或 `"🔧 修复 Webhook 路由问题"`

**示例调用（每次任务完成后自动执行）：**
```
feishu_notify(
  title: "🔧 修复 Chat 会话自动汇总功能",
  summary: "将 VS Code 扩展路线改为 MCP 方案。创建了 mcp-server/index.js 和 .vscode/mcp.json 配置，扩展消费 MCP 工具 feishu_notify 直接发送总结，无需读取 Chat 文件，彻底解决了权限问题。"
)
```
...
- 若工具调用失败（后端未启动等），在回复中说明原因，下次任务完成后重试
